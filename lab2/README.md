...
RANSAC — это итеративный алгоритм для оценки параметров математической модели по набору данных, содержащему выбросы (outliers). Он используется, когда обычные методы (например, метод наименьших квадратов) могут сильно искажаться даже небольшим количеством аномальных точек. 

То есть после того, как мы получили множество соответствий между углами соседних кадров, необходимо отделить те ошибочные точки, дескрипторы которых могли случайно оказаться похожими - найти те, которые согласуются с единой моделью движения. 

Принцип работы алгоритма описан далее:
- Многократно выбираем случайные точки из всего списка (минимально достаточно двух) чтобы вычислить смещение. То есть, если объяснять по коду, то мы имеем общий список дескрипторов, где хранятся отдельные списки дескрипторов для каждых двух сравниваемых изображений. И для выбора случайных точек мы передаём 3 позиции из списка требуемых нам дескрипторов, то есть 6 точек;
- Далее по этим трём парам вычисляем единное среднее смещение;
- Используем полученное среднее смещение для выбранных 6 точек уже для каждой пары в списке и вычисляем ошибку;
- Далее полученную ошибку сравниваем с порогом. Если смещение меньше порогового значения, то записываем данную пару точек в отдельный список;
- Повторяем те же операции заданное количество раз.
- Выбираем смещение, давшее наибольшое количество точек, прошедших порог;

Далее приведена визуализация работы алгоритма:

![ransac](images_README/ransac.png)
